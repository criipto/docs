...
1. [Adding your own stylesheet](#custom-css-adding-a-stylesheet): if your goal is to apply basic styling, including a custom CSS file is all you need.
2. [Adding custom HTML and CSS](#custom-html): can be used for creating more complex UIs (e.g. a layout with extra information around the Idura login box).
...
Strict **Content-Security-Policy** rules apply to all kinds of custom styling of Idura Verify dialogs.
...
You can always use the built-in `Try` feature for the various eID methods to run an actual login flow in the browser, where `Content-Security-Policy` rules will apply.
...
You can add your own stylesheet from the **Styling** -> **General** tab of your [Idura Dashboard](https://dashboard.idura.app/styling).

<Highlight icon="info">
  Your custom CSS is compiled using [PostCSS](https://postcss.org/) with
  [Autoprefixer](https://github.com/postcss/autoprefixer), [PostCSS Preset
  Env](https://preset-env.cssdb.org/), and [PostCSS
  Nesting](https://www.npmjs.com/package/postcss-nesting) plugins enabled. This means you can use
  modern CSS features without worrying about browser compatibility.
</Highlight>
...
In some cases, bundling all of your CSS in just one style sheet can be tricky and/or infeasible. Idura Verify supports overriding the _path_ part of the style sheet URL on a per-authorize request basis.
...
Idura Verify will use the authority part of your pre-configured URL as a "base" URL, and then set the value you specify in the `css_path` token as the path name and query part of the style sheet URL.

<Highlight icon="file-lines">

If you have not configured a custom style sheet URL, Idura Verify ignores the `css_path` token in the `login_hint` query parameter.
...
<Highlight icon="info">
 The text elements in the last 3 paragraphs in the `<aside>` are intentionally left blank by default.
</Highlight>
...
The first selector hides all the default texts in the `<aside id="description">` element in one go, and the subsequent 4 selectors add language-specific texts per `<p>` element.

If you want to show text in one of the hidden-by-default `<p>` elements, you can do the following (here, we add English text to the third `<p>` element):
...
<Highlight icon="info">
 The first selector is needed to "undo" the effect of the `default-hidden` class on the third `<p>` element.
</Highlight>
...
2. Indicator that this is a page from Idura Verify - `broker`. This makes it possible to target our pages specifically, even if you use the same CSS file for different websites.
3. Indicator of the flow in use - authentication (`auth`) or signature (`sign`).
4. Indicator of which DNS name the page is loaded on.
5. (Optional) Your very own CSS class name on a per-application basis. You can set it up in the **UI** section of your application settings in the [Idura Dashboard](https://dashboard.idura.app/).
...
You can use these dynamically added CSS classes to customize the UI on a per-application basis, which may be quite practical for, say, Single Sign-On scenarios where you reuse an Idura Verify DNS domain across web applications with very different branding requirements.
...
You can create your preferred webpage structure around the login box on the Idura login screens by adding custom HTML (with accompanying CSS).

Access this feature from the **Styling** -> **Custom UI** tab of your [Idura Dashboard](https://dashboard.idura.app/styling).
...
Your HTML must always include the `<verify />` element, as this will define the position of the Idura login box on the login screen.
...
For inspiration, take a look at [various examples of custom UIs](/verify/reference/samples/custom-ui-samples).

### Feedback

We're working on adding different versions of the custom HTML depending on the selected eID.

Your feedback on this feature is highly appreciated. Don't hesitate to reach out via [Slack](https://tiny.cc/criipto-slack) or [email](mailto:support@idura.eu) with any suggestions or comments.
