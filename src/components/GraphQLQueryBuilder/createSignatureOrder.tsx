...
import { query } from '../../examples/createSignatureOrder.graphql';
import {
...
export default function CreateSignatureOrderQueryBuilder() {
  ...
  const variables = (): { input: CreateSignatureOrderInput } => {
    return {
      input: {
        ...input,
        documents,
  ...
  const handleChangeDocument = (document: DocumentInput, key: string, value: string) => {
  ...
  const handleRemoveDocument = (document: DocumentInput) => {
    setDocuments(documents => documents.filter(d => d !== document));
  };
      ...
      <div className="mb-4 grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <input
            ...
            value={input?.maxSignatories || ''}
            onChange={event =>
              setInput(input => ({
                ...input,
                maxSignatories: parseInt(event.target.value, 10),
              }))
            }
          />
        </div>
        <div>
          ...
          <input
            ...
            value={input?.expiresInDays || ''}
            onChange={event =>
              setInput(input => ({
                ...input,
                expiresInDays: parseInt(event.target.value, 10),
              }))
            }
          />
          <small>
            Old signature orders that are not cancelled or closed are automatically cleaned up.
          </small>
        </div>
        <div>
          <label
          ...
          <input
            ...
            value={webhook?.url || ''}
            onChange={event => setWebhook(input => ({ ...input, url: event.target.value }))}
          />
          <small>You can use https://webhook.site/ for testing</small>
        </div>
      </div>
      <H3>UI Settings</H3>
        ...
        <div>
          ...
          <input
            ...
            onChange={event =>
              setUI(ui => ({
                ...ui,
                signatoryRedirectUri: event.target.value,
              }))
            }
          />
        </div>
