...
After implementing the changes described in the [technical guides](/verify/e-ids/finnish-trust-network/#technical-guides), you can test your setup by activating the **Enforce FTN requirements** toggle. You'll find it in your Criipto Dashboard under `eID providers` > `FTN`.

We recommend activating the toggle in a test environment, then running a test login with FTN. When active, this setting will make non-compliant authorization requests fail. The failure will include a detailed error message, helping you identify any misconfigurations or missing steps in your implementation.
...
const createSignedRequestObject = async (signingKey) => {
  ...
  const signedJwt = await new SignJWT(requestObjectPayload)
    .setProtectedHeader({
      alg: 'RS256',
      kid: SIGNING_KEY_KID,
    })
...
After receiving the JWE, use the private key of your encryption RSA key pair to decrypt it and retrieve the original signed JWT.

The code below uses `jose` library for JWE decryption and `@criipto/oidc` to handle code exchange.

```javascript
const { createRemoteJWKSet, compactDecrypt, jwtVerify } = require('jose');
...
const jwks = createRemoteJWKSet(new URL(`https://${CRIIPTO_DOMAIN}/.well-known/jwks`));
  ...
  try {
    ...
    if (id_token) {
      ...
      try {
        ...
        const { plaintext: idTokenSignedJwt } = await compactDecrypt(
          id_token,
          privateKeyForDecryption
        );

        // 4. Verify the decrypted id_token's signature.
        const { payload: idTokenClaims } = await jwtVerify(idTokenSignedJwt, jwks, {
