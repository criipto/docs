...
subtitle: How to control the session behavior of your Idura Verify domain
---

Enabling Single Sign-On, SSO, for an Idura Verify domain will let your users get access to several of your systems with a minimum of effort. They will only be prompted for a login once per browsing session when you are using SSO. Subsequent authentication requests on the same domain (and for the same eID method) will be completed automatically by Idura Verify, thereby reducing the UX friction.

<Highlight icon="file-lines">

The session information needed to maintain an SSO session is maintained in `HTTP-only`, `Secure` cookies, created with `SameSite=None`. This means that the session can be established and maintained even if your users access your Idura Verify domain in third-party context. Cookies and therefore also the SSO session is tied to a single Idura Verify domain.

</Highlight>

## How to enable Single Sign-on

You enable and disable `SSO` per DNS domain through the management dashboard for Idura Verify. Go to the `Domains` tab on [dashboard.idura.app](https://dashboard.idura.app/domains) and click on the domain you choose for SSO. The following options are available:
...
- **Silent check**: Check if the user has an existing session. Idura Verify will never require any user interactions in this case. You specify this by adding an extra query parameter `prompt=none`.
