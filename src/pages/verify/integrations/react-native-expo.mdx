...
- _Client ID_ to identify your application to Idura Verify.
- _Domain_ on which you will be communicating with Idura Verify.

### Register callback URLs

Before you can start sending authentication requests to Idura Verify, you need to register the URLs on which you want to receive the returned _JSON Web Token_, [JWT](verify/reference/token-contents/).
...
If you wish to support [app switch with Danish MitID](#app-switch-support), you must use <a href="https://docs.expo.dev/linking/overview/#universal-linking" target="_blank">universal/app links</a>.
...
Setup the Idura Verify SDK by wrapping your application in `CriiptoVerifyProvider`:
...
export default function App() {
  return (
    <CriiptoVerifyProvider domain="{{YOUR_IDURA_DOMAIN}}" clientID="{{YOUR_CLIENT_ID}}">
      <View>
        <LoginScreen />
        ...
      </View>
...
- `claims` variable that allows to retrieve information about the user's [JWT claims](/verify/reference/token-contents/) once they are available.
- the `logout` method.
...
<a href="https://docs.expo.dev/linking/overview/#universal-linking" target="_blank">
