...
You must enable the "Add CPR for MitID logins" toggle on the [Danish MitID](https://dashboard.idura.app/providers/DK_MITID) eID provider screen on the Idura Dashboard.
Idura Verify will prompt the user for their CPR number and validate that it belongs to the MitID user that is logging in.

<Highlight>
  Not all MitID users have a CPR number. If your application can handle the case of a missing CPR,
  you may enable the `CPR Optional` toggle. This will let MitID users without a CPR number log in to
  your service.

![CPR Optional toggle](./images/mitid-cpr-optional-toggle.png)
...
- The person applying must either be enrolled in MitID Erhverv for your company, or be a signatory of the company.
...
In addition to the general [Idura terms of service](https://idura.eu/legal/tos), you must also accept the [MitID specific terms](https://idura.eu/legal/mitid/tos).
...
1. Go to the [management dashboard](https://dashboard.idura.app) and set the environment toggle at the top left to "Production environment".
2. Navigate to the "eID providers" page and go to "DK MitID".
...
- The name in the "Company alias" field will be displayed to end-users as the [MitID service provider alias](/verify/e-ids/danish-mitid/#mitid-service-provider-alias).
...
1. Send an email to <a href="mailto:orders@idura.eu">orders@idura.eu</a> with your desired domain prefix.

- The domain prefix is typically your company or brand name, e.g. acme-corp. This will be used to set up your MitID domain, in this case acme-corp.mitid.dk.

- All custom MitID domains must end in `.mitid.dk`

2. The information is then forwarded to Nets who will set up your MitID domain.

- Expect this process to take 15 business days.
...
### MitID service provider alias

When [setting up your MitID service provider](/verify/e-ids/danish-mitid/#apply-for-production-access), you are asked to pick an `alias`.

The MitID service provider alias will be displayed in the web login box (see screenshot below) and in the app when the user approves the login/transaction.

<ImageContainer maxWidth={300}>

![Service Provider ALIAS on web](./images/mitid-service-provider-alias-web.png)
