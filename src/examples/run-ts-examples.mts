...
const examples = filename
  ...
  : glob(import.meta.dirname + '/**/*.node.ts');
...
for await (const example of examples) {
  ...
  try {
    ...
    new Worker(src, {
      eval: true,
      stdout: true, // Passing true means "direct the output of the worker to a stream, instead of piping to process.stdout"
    })
      .on('error', reject)
      .on('exit', resolve)
      .stdout.on('data', data => stdout.push(data));
    await promise;
