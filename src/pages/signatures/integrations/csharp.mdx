...
sort: 1
title: .NET (C#)
subtitle: Integrate Idura Signatures with C#/dotnet to sign PDFs using MitID, Swedish BankID, Norwegian BankID or other eIDs.
---

This guide shows you how to use implement Idura Signatures in .NET to sign
PAdeS-LTA documents using MitID, BankID or any other eID supported by Idura Verify.

## Getting started

### Requirements

This library supports netstandard2.0 (including .NET 8 and .NET 10)
...
```csharp
var client = new CriiptoSignaturesClient("{YOUR_IDURA_CLIENT_ID}", "{YOUR_IDURA_CLIENT_SECRET}");
```
...
using (var client = new CriiptoSignaturesClient("{YOUR_IDURA_CLIENT_ID}", "{YOUR_IDURA_CLIENT_SECRET}")) {
...
More [.NET/C# examples](/signatures/graphql/examples/) are provided for all the options supported by the Idura Signatures API.
