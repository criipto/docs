...
Evidence validation keys must match what will be found in the token claims when signing. You can inspect the [JWTs issued by Idura Verify](/verify/reference/token-contents/) to see the keys that can be used for evidence validation. _Tip: Hover over a claim in the JWT payload to view its description._
