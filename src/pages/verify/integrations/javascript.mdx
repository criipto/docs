...
This guide shows how to use Idura Verify in a JavaScript application with plain HTML.
...
After you signed up for Idura Verify, you must [register an application](https://dashboard.idura.app/applications/add) before you can try logging in with any eID.

Once your application is registered, the dashboard will contain the necessary details to configure your application to communicate with Idura Verify.
...
<script type="text/javascript" src="https://unpkg.com/@criipto/auth-js@^3.8"></script>
```
...
- `domain (required, string)`: The domain your Idura Application belongs to, such as `example.idura.broker`.
- `clientID (required, string)`: The Client ID for your Idura Application.
...
- [`criiptoAuth.redirect.authorize`](#authorization-with-redirect) - Redirects the user to the Idura Verify login page.
...
Will redirect the user to the Idura Verify login page.
...
The `redirectUri` should be [whitelisted in the "Callback URLs" section](#register-callback-URLs) in your Idura application settings.
