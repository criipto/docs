...
- For [confidential clients](/verify/getting-started/glossary/#confidential-client) (traditional server-based web applications), `private_key_jwt` client authentication is mandatory.
- [Public clients](/verify/getting-started/glossary/#public-clients) (native apps or SPAs) cannot securely store private keys, and therefore must continue using [PKCE](/verify/getting-started/glossary/#pkce-proof-key-for-code-exchange) for securing the authorization code flow.

PKCE is also highly recommended for confidential clients as an additional layer of security. See [OpenID Connect best security practices](/verify/getting-started/best-security-practices/#authorization-code-flow) for more information.

Follow [this guide](/verify/guides/privatekey-jwt/) to configure Private Key JWT authentication. _Note: You must use Statically configured JWK sets._
