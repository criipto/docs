...
subtitle: How to use private key JWT to authenticate your Idura application
---
...
You can configure your [Idura Verify Application](/verify/getting-started/basics/#applications) to use `private_key_jwt` for client authentication.
...
3. Use the client assertion to authenticate your application against Idura Verify.

You can use one of the recommended libraries for working with JWTs (the list is available at [jwt.io](https://jwt.io/libraries)) or Idura's [criipto-oidc-js](https://github.com/criipto/criipto-oidc-js) library. We strongly recommend utilizing an established tooling or a third-party library rather than building your own solution.
...
Ideally, you would manage and expose your keys via a JWKS endpoint, but for the purposes of this guide, we will work with a static JWK Set.
...
When we configure PKJWT to create client assertion, this is a signing key.
We also need to create an encryption key to validate the token.

## Configure Private Key JWT Authentication

To configure Private Key JWT Authentication, you'll need to register JWKS on behalf of your client on [dashboard.idura.app](https://dashboard.idura.app).
...
The input must look something like this. Note that you must include both a `'sig'` and an `'enc'` key.
  ...
  "keys": [
    {
      "kty": "RSA",
      "use": "enc",
      "kid": "your-encryption-key",
      "e": "AQAB",
      "n": "zwEbepsvbl4TpZNecCtmJnMhALAH0sqlpllZdkI6AsRGou_cxakr8b8HKjjYWiyb-RJzbScGxBLPVK4k9RcEOL3PxoGoSIABEE1-NinDoiTYaSRIYwd7N914vWSGgfs7ctMy7vYRXmTBoc_NuLQkl79Fu2tFjXbLyAgGfnePkZUrFJqCXwCB553QYXJ0Lw4B8P8MFjy80YShI1wBQmCfj5BpZbACN5u5-84233KTHqYIkGOTrF4FZwVRvAkNutjiwMIzwKYGRj2qjdmelmqDlgiPre5UP8Cfnl7Urm411JhueWo7mrvJAcwHMpcyE4Rsuyhm93bv0hBDe5ddt-r4cw"
    },
    {
      "kty": "RSA",
      "use": "sig",
      "kid": "your-signing-key",
      "e": "AQAB",
      "n": "0TGwj4SywrEU3IYBBm-HoxyI7zKQkjM6Y4PEZNNBmoIFk4kU8J5xVNkbkqnv_SwKrKJD3JbYK18zLvyu05xVDxXEZR8FkipGaXisGOuXuo_A8Gswe_-LLYfN3N-bpcID9E8FnSq6todIO8umD50W9VdGSoO_WxBiffnpnpXDihTuGSLLSdhWz6vz7-VY_Uphv6V87FfqU0OaMGLi7InsZlMSlquvb8LthOm22YvuiRkgkX1cMjW-uaWva7AMQJ6eKbOKRADnMGNjwlUOJkPsqUbCUGuMvNFwqh9JQCBHEvnwGz8Lmt8hNaK0uQ0nT3l2PFZ5_LsH9JrpxqCmD2LHeQ"
    }
  ]
}
```
...
Instead, you generate a pair of asymmetric keys and have Idura point to the JWKS in your system. During authentication, your application signs its request with the private key, creating a signed client assertion. Idura Verify can then use the public key from the JWKS to decrypt and verify the assertion.

Additionally, by using `private_key_jwt` with a _JWKS endpoint_ rather than a static JWKS, you can benefit from seamless key rollover by eliminating the need to make any changes in the Idura Dashboard if you choose to rotate your keys.

<Highlight icon="file-lines">

With `client_secret`:

Idura Verify generates the `client_secret`. Every time the application wants to perform client authentication, it must send the `client_secret` in order to obtain an access token.

With `private_key_jwt`:

You generate your own asymmetric key pair and register it with Idura Verify by supplying your JWKS. Therefore, the private key is under your control and is never shared or transmitted over the network.
