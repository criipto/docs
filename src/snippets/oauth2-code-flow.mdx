...
2. Copy the generated client secret. _Note that this is the only time you will be shown the actual value of the client secret. Idura only stores this as a hashed value, which means you cannot retrieve the value once it has been generated and stored._
...
- Signed JWT (`signedJwt`): User information is returned from the `userinfo` endpoint as a digitally signed [JSON Web Token](/verify/reference/glossary/#jwt-json-web-token).
- Signed and encrypted JWT (`signedAndEncryptedJwt`): User information is returned from the `userinfo` endpoint as a signed and encrypted [JSON Web Encryption(JWE)](/verify/reference/glossary/#json-web-encryption-jwe) object.
