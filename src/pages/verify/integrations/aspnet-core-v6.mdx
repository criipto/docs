...
subtitle: Accept MitID, Swedish BankID, Norwegian BankID and other eID logins with ASP.NET Core 6.0 and Idura Verify
---

This tutorial demonstrates how to integrate Idura Verify into a fresh ASP.NET Core 6.0 application created with `dotnet new mvc`.
...
## Register Your Application in Idura Verify

After you signed up for Idura Verify, you must [register an application](https://dashboard.idura.app/applications/add) before you can actually try logging in with any eID.

Once you register your application, you will also need some of the information for communicating with Idura Verify. You get these details from the application settings in the dashboard.

Specifically you need the following information to configure you application

- _Client ID_ to identify you application to Idura Verify.
- _[Domain](/verify/getting-started/basics/#domains)_ on which you will be communicating with Idura Verify.
- _Client Secret_ to perform code exchange.

### Register callback URLs

Before you can start sending authentication requests to Idura Verify, you need to register the URLs on which you want to receive the returned _JSON Web Token_, JWT.

The Callback URL of your application is the URL where Idura Verify will redirect to after the user has authenticated in order for the OpenID Connect ASP.NET middleware to complete the authentication process.
...
To integrate Idura Verify with ASP.NET Core, you will use the Cookie and OpenID Connect (OIDC) authentication handlers. `Microsoft.AspNetCore.Authentication.Cookies` is usually included but `Microsoft.AspNetCore.Authentication.OpenIdConnect` will need to be installed:
...
The OIDC middleware requests both the `openid` and `profile` scopes by default, you may configure additional scopes if your application is [configured with dynamic scopes](/verify/getting-started/oidc-intro/#the-scope-parameter).
...
.AddOpenIdConnect(options => {
    ...
    // The next to settings must match the Callback URLs in Idura Verify
...
Logout requires both terminating the local session by removing the cookies as well as telling Idura Verify that the session is over.
...
3. The user's browser is redirected to the Idura Verify service, where the actual login takes place.
4. A callback with an issued _authorization code_ is sent back to the application and intercepted by the OIDC middleware.
5. The middleware calls the Idura Verify service to exchange the code for an _access token_. Note that this is a direct server-to-server call which, unlike the other communication, does not pass through the browser.
