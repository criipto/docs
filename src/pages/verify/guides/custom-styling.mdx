...
You can add your own stylesheet from the `Styling` -> `General` tab of your management dashboard on [dashboard.idura.app](https://dashboard.idura.app/styling).
...
**_Note_** The text elements in the last 3 paragraphs in the `<aside>` are intentionally left blank by default.
...
**_Note_** The first selector is needed to "undo" the effect of the `default-hidden` class on the third `<p>` element.
...
This can be done from the `Styling` -> `Custom UI` tab of your management dashboard on [dashboard.idura.app](https://dashboard.idura.app/styling).
