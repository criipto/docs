import { glob, readFile, writeFile, rm, mkdir, cp } from 'node:fs/promises';
import { isAbsolute, join } from 'node:path';
import { spawnSync } from 'node:child_process';
import { parseArgs } from 'node:util';

const {
  values: { quiet },
  positionals: [filename],
} = parseArgs({
  allowPositionals: true,
  options: {
    quiet: {
      type: 'boolean',
      short: 'q',
    },
  },
});
const CRIIPTO_SIGNATURES_CLIENT_ID =
  (process.env['CRIIPTO_SIGNATURES_CLIENT_ID'] as string) ??
  raise('CRIIPTO_SIGNATURES_CLIENT_ID missing');
const CRIIPTO_SIGNATURES_CLIENT_SECRET =
  (process.env['CRIIPTO_SIGNATURES_CLIENT_SECRET'] as string) ??
  raise('CRIIPTO_SIGNATURES_CLIENT_SECRET missing');

const examples = filename
  ? [isAbsolute(filename) ? filename : join(import.meta.dirname, filename)]
  : glob(import.meta.dirname + '/**/*.cs');

const dist = join(import.meta.dirname, '../../dist/signatures-csharp');

await rm(dist, { recursive: true, force: true });
await mkdir(dist, { recursive: true });
await cp(join(import.meta.dirname, 'examples.csproj'), join(dist, 'examples.csproj'));

for await (const example of examples) {
  process.stdout.write(`Running ${example}`);

  const src = (await readFile(example, 'utf-8'))
    .replace('{YOUR_CRIIPTO_CLIENT_ID}', CRIIPTO_SIGNATURES_CLIENT_ID)
    .replace('{YOUR_CRIIPTO_CLIENT_SECRET}', CRIIPTO_SIGNATURES_CLIENT_SECRET)
    .replace('validateConnectivity = true', 'validateConnectivity = false') // httpbin periodically times out, so don't validate connectivity
    .replaceAll(
      `new Byte[64]`,
      `Convert.FromBase64String("")`,
    );

  try {
    await writeFile(join(dist, 'Example.cs'), src);

    const { status, stdout, stderr } = spawnSync('dotnet', [
      'run',
      '--project',
      join(dist, 'examples.csproj'),
      '--framework',
      'net8.0',
    ]);

    if (status === 0) {
      process.stdout.write(' ✅\n');
    } else {
      process.exitCode = -1;
      process.stdout.write(' ❌\n');
    }

    if (!quiet) {
      process.stdout.write(stdout.toString().trim());
      process.stderr.write(stderr.toString().trim());
      process.stdout.write('\n');
    }
  } catch (error) {
    process.exitCode = -1;
    process.stdout.write(' ❌\n');
    if (!quiet) {
      process.stderr.write((error as Error).toString());
    }
  }
}

function raise(message: string) {
  throw new Error(message);
}
