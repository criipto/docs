...
Idura will apply [JSON Web Encryption (JWE)](/verify/reference/glossary/#json-web-encryption-jwe) to secure `token` and `userinfo` responses resulting from FTN authentication.
This means that after a successful [code-for-token exchange](/verify/reference/glossary/#code-exchange), the `id_token` (and `userinfo` if requested) will be sent to your application in a JWE format, rather than a standard [JWT](/verify/reference/glossary/#json-web-token-jwt).
...
- [An Introduction to JSON Web Encryption](https://www.idura.eu/blog/json-web-encryption-introduction)
- [Securing Authentication Flows with JSON Web Encryption](https://www.idura.eu/blog/jwe-encrypted-responses)
...
A legal agreement with Idura is mandatory to access FTN. You can request one by filling out the <a href="https://dashboard.idura.app/providers/FI_TELIA/contract" target="_blank"> FTN Production Agreement form </a> in your Idura Dashboard. The whole process typically takes 2-3 business days.
