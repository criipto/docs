...
you can configure the Idura Verify evidence provider to enforce uniqueness via the `uniqueEvidenceKey: 'sub'` setting.
...
Evidence validation keys must match what will be found in the token claims when signing. You can
inspect the [JWTs issued by Idura Verify](/verify/reference/token-contents/) to see the keys that
can be used for evidence validation. _Tip: Hover over a claim in the JWT payload to view its
description._ Note, that not all claims are included by default. In particular, you must request the
`ssn` scope, in order to validate on SSN.
...
### Define whom the signatory is signing as

You can define whom the signatory is signing as to differentiate between functions, if perhaps the same eID is used for signing multiple times.

import addSignatorySigningAsExample from '../../../examples/addSignatory/signingAs';

<GraphQLExample example={addSignatorySigningAsExample} />
...
### Custom seal position with scaling

In certain situation it might be ideal to be able to define a smaller sized signature seal.
A `scale` can be defined as part of a custom seal position.
The `scale` goes between 0.2 and 1.0, both included, in increments of 0.1.
Any provided number in that range is rounded down to the nearest 0.1.

Same definition as shown in [Custom seal position](/#custom-seal-position) is expected.

import addSignatorySealPositionScaledExample from '../../../examples/addSignatory/sealPositionScaled';

<GraphQLExample example={addSignatorySealPositionScaledExample} />
...
In some cases you might want to enforce a signing sequence, e.g., on a rental contract.
In such a case, you are able to define the signing sequence number for a signatory where the lowest number in the sequence needs to sign before others.
If multiple signatories have defined the same signing sequence number, they can sign in parallel.

The sequence number must be a positive integer. It defaults to the maximum int32 value.

If you want to patch in signatories along the way, it might be a good idea to define intervals, e.g. signatory A is 5, signatory B is 10 and signatory C is 15.

import addSignatorySigningSequence from '../../../examples/addSignatory/signingSequence';

<GraphQLExample example={addSignatorySigningSequence} />

### Signatory Role

NB: An in-depth guide can found under [Guides > Signatory Roles](/signatures/guides/signatory-roles/)

A signatory's role can be defined to specify what responsibility the given signatory has.

import addSignatorySignatoryRole from '../../../examples/addSignatory/signatoryRole';

<GraphQLExample example={addSignatorySignatoryRole} />
...
This feature currently ONLY works with Verify-based signatures.
...
The PDFs are deleted from Idura servers upon calling `closeSignatureOrder`. (Unless you define the `retainDocumentsForDays` setting.)
...
If you wish, you can ask Idura to retain the documents for up to 7 days after closing the signature order:
