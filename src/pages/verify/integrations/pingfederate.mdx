...
sort: 3
title: PingFederate
subtitle: This tutorial demonstrates how to integrate Idura Verify with PingFederate
---
...
1. [Create an application in Idura Verify that represents your PingFederate tenant](#create-idura-verify-application)
2. [Create an authentication source in your PingFederate tenant which connects to the Idura Verify application](#create-pingfederate-authentication-source)
3. [Complete the application configuration in Idura Verify](#complete-the-application-configuration-in-idura-verify)
...
which _is_ a bit of a chicken-and-egg-problem, unfortunately. We suggest that you break the deadlock by configuring a temporary (bogus) callback URL in the first step in Idura Verify, and then replace it with the actual value available after the authentication source is created.
...
- **Client secret** which PingFederate needs to fetch actual user information from Idura Verify during login. The secret is generated when you [configure the OAuth2 code flow](/verify/integrations/pingfederate/#configure-the-oauth2-code-flow) for your application.
- **Domain** on which you will be communicating with Idura Verify. We'll use `acme-corp.idura.broker`.

![Create App](./images/pingfederate-create-application.png)
...
Give the connection a recognizable name, copy-paste the _Client ID_ and _Client secret_ values from your Idura Verify application
...
You can find the available claim types [here](/verify/reference/token-contents).
...
You can read more about which per-authorize-request parameters you can use to control the runtime behavior of Idura Verify [here (prefilled fields)](/verify/guides/prefilled-fields) and [here (acr_values)](/how-to/choose-eid-method#login-hint-embedded).
